...
| **Auth**                    | `GCP Workload Identity`    | Secure, keyless authentication to Google Cloud (GKE) eliminating the risk of long-lived service account keys.                |
| **Deployment**              | `Google Kubernetes Engine` | Deploys the application to a managed Kubernetes cluster with auto-scaling and high availability.                             |
| **DAST (Dynamic Analysis)** | `OWASP ZAP`                | actively attacks the running application to find vulnerabilities (e.g., XSS, broken access control) in the live environment. |

## 4. Expected Outcomes

- Fully automated build, test, and deployment process on every commit to main.
...
- Live application running on GKE protected by dynamic security scans.
